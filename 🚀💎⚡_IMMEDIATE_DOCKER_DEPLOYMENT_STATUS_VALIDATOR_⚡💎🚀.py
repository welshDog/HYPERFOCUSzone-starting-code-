    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if len(lines) > 1:  # Header + at least one container
                ...
                print("⚠️ No Docker containers currently running")
                return 0
        else:
            print("❌ Docker command failed or not accessible")
            return -1
...
def check_ai_services():
    ...
    print("🧠 Checking AI and monitoring services...")
...
def generate_victory_summary():
    ...
    print("\n" + "="*60)
    print("🎊💎⚡ DOCKER DEPLOYMENT VICTORY SUMMARY ⚡💎🎊")
    print("="*60)
    ...
    if accessible_count >= 1:
        print("   ✨ AI Infrastructure Deployed")
    if accessible_count >= 2:  
        print("   🚀 Monitoring Stack Active")
    if accessible_count >= 3:
        print("   🧠 Cognitive Enhancement Achieved")
    if container_count >= 40:
        print("   🏭 Enterprise-Level Infrastructure")
    
    print(f"\n🕐 Status Check: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("💎 Victory preserved in Memory Crystals!")
    print("🎊 EMPIRE STATUS: AI-POWERED & LEGENDARY READY!")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n❌ Validation error: {e}")
        print("🔧 Manual status check may be needed")
